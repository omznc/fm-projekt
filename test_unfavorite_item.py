...
def test_unfavorite_item(browser):
    """
    Unfavoriting an item works correctly.
    :param browser:
    """
    ...
    try:
        ...
        assert (
            "Uspješno ste izbrisali oglas iz spašenih" in success_message.text
        ), "Expected 'Uspješno ste izbrisali oglas iz spašenih' message"
    except TimeoutException:
        print("The toast message did not appear within 10 seconds.")
        assert False, "TimeoutException was raised"
    ...
    assert "Spasi oglas" in save_button_new.text, "Expected 'Spasi oglas' button text"
